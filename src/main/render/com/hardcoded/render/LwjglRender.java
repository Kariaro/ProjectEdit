...
import com.hardcoded.render.gl.RenderUtil.DepthFunc;
...
public class LwjglRender {
		...
		if(resourceLoader.shouldRender()) {
			...
			if(Input.isControlDown()) {
				...
				if(Input.isKeyDown(GLFW.GLFW_KEY_1)) {
					...
					//					camera.rx = 180;
					camera.x = -6.801223f;
					camera.y = 331.729810f;
					camera.z = -126.773783f;
					camera.rx = 0.000000f;
					camera.ry = 90.000000f;
				}
				...
				if(Input.isKeyDown(GLFW.GLFW_KEY_3)) {
					camera.x = 1702.766724f;
					camera.y = 139.736618f;
					camera.z = -2339.164307f;
					camera.rx = 151.000000f;
					camera.ry = 67.750000f;
				}

				if(Input.isKeyDown(GLFW.GLFW_KEY_9)) {
					System.out.printf(Locale.US,
							"camera.x=%ff;camera.y=%ff;camera.z=%ff;camera.rx=%ff;camera.ry=%ff;\n", camera.x, camera.y,
							camera.z, camera.rx, camera.ry);
				}
			}
	...
	void drawRegionGeometry(DynamicMeshBuffer buffer, List<IRegion> copy) {
		...
		float[] color_odd = { 0.7f, 0.7f, 0.7f, 1.0f, 0.7f, 0.7f, 0.7f, 1.0f, 0.7f, 0.7f, 0.7f, 1.0f, 0.7f, 0.7f, 0.7f,
				1.0f, 0.7f, 0.7f, 0.7f, 1.0f, 0.7f, 0.7f, 0.7f, 1.0f };
		float[] color_even = { 0.5f, 0.5f, 0.5f, 1.0f, 0.5f, 0.5f, 0.5f, 1.0f, 0.5f, 0.5f, 0.5f, 1.0f, 0.5f, 0.5f, 0.5f,
	...
	void drawFrustumChunks(World world, Matrix4f projectionView, Vector3f camera, int radius) {
		...
		for(int i = xs; i <= xe; i++) {
			for(int j = zs; j <= ze; j++) {
				...
				for(int k = 0; k < 16; k++) {
					if(intersect.testAab(new Vector3f(i * 16, k * 16, j * 16),
							new Vector3f((i + 1) * 16, (k + 1) * 16, (j + 1) * 16))) {
						if(chunk.getStatus() != Status.LOADED) {
